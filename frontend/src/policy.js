const policy_aaai2022 = String.raw`
# SUPERB Challenge
...
const policy_slt2022 = String.raw`
...
* benchmarks **generalizability** of **Self-Supervised Learning (SSL)** speech model.
    ...
    * Using various **Downstream** tasks
* has several [downstream tasks](#Tasks)
    - **Content**: PR, ASR, QbE
    - **Speaker**: SID, ASV, SD
    - **Paralinguistics**: ER
    - **Semantics**: ST $\star$
    - **Generation**: SE $\star$, SS $\star$
* has two leaderboards to submit:
    * **Public-set**: Dataset is publicly available, submit **prediction file**
        ...
        * Except above, no other limitations in training procedure (e.g. hyperparameter, optimizer, etc.)
    * **Hidden-set**: Dataset is totally inaccessible, submit **model** (model definition & pre-trained weights)
        * Several fixed training procedures (usually are just different in learning rate) and present the best result.
* encourage innovative and aims to be less competitive:
    * Allow anonymity in model and organization name, but need the identity of people who submited the model (public-set submission through github)
    * Add number of parameters and operations as metrics, encourage improvement of different perspectives
...
![](https://i.imgur.com/AfzTvhc.png)
*Fig 2.*

Fig 2. illustrates the evaluation framework of the challenge. The challenge evaluates SSL models' generalizability on 10 [tasks](#Tasks). Each of the tasks has a corresponding public dataset (**public-set**) that is publicly available, and a hidden dataset (**hidden-set**) that will not be released. Participants can practice on the public-set to understand the performance of their upstream models, and choose the best one for submission as they wish. Then, participants **submit the upstream model** (model definition & pre-trained weights) publicly or privately to the hidden-set leaderboard ([submit tutorial](https://huggingface.co/superb/superb-submission)FIXME). **We finetune the downstream models on the hidden-set** without releasing any audio/label. Both public-set and hidden-set have leaderboards and welcome submissions to share more results with the community. Finally, there will be **overall metrics** for ranking all upstreams.

All the participants are encouraged to submit papers to [*The IEEE Workshop on Spoken Language Technology (SLT)*](https://slt2022.org/). And optionally, participants may also choose to submit system description papers to a separate Challenge Proceedings. Review of these papers will be the responsibility of our SUPERB challenge organisers. Accepted system description papers will not be indexed by the IEEE, but authors of these
papers will have the opportunity to present their work in a dedicated session at the Workshop. 
...
- **New Metrics**: 
    - **Params**: The count of upstream model parameters.
    - **MACs**: The estimated number of Multiply ACcumulate operations while **forwarding**.
        - Estimated by $\max\{num(add)+num(sub),\ num(mul)+num(div)\}$
...
We accept upstream models in PyTorch by default. If you wish to submit upstreams in non-PyTorch frameworks, please [contact us](#Contact)!
If you are not feasible to submit the pre-trained model, please [contact us](#Contact) for us to see how to help!
...
The datasets used in the public-set are all chosen to be public available for everyone to participate. Please refer to [TASKS](https://superbbenchmark.org/tasks) and the implementation in [S3PRL](https://github.com/s3prl/s3prl/blob/master/s3prl/downstream/docs/superb.md) for details.
...
The [public-set leaderboard](leaderboard?subset=Public+Set&track=constrained) is online and [accepts submissions](submit?type=public). There is no deadline. Since all the train/dev/test splits are public available, **the leaderboard accepts submissions with the inferenced prediction files on each task's testing split** which will be auto-generated if you follow the benchmarking steps in [S3PRL](https://github.com/s3prl/s3prl/blob/master/s3prl/downstream/docs/superb.md). And in this challenge, we only consider submissions submited to the **Constrained track** ([here](/rules) is further about track). 

## Hidden-set

### Fairness & prevent overfitting

Since all the train/dev/test splits are public in the public-set, it is possible to cheat by directly reporting the best results on the testing split, and the results are thus overfit on the testing split. Hence, the hidden-set is collected and prepared to follow the same task design as that in the public-set but with the newly created data. All the splits will **NOT be released in both audio and labels**. The members involved in the hidden-set preparation should **NOT** participate the challenge. These members are listed in the [**Hidden-set Committee**](#Hidden-set-Committee) below.

### Hidden-set leaderboard and submission

#### Submission type

The leaderboard accepts **submissions with the upstream model solely**, including **model definition** and **pre-trained weights**. The upstream model should follow the specification detailed at [Upstream Specification](#Upstream-Specification). The submission can be done publicly or privately. Only the [**Hidden-set Committee**](#Hidden-set-Committee) members can access the privately submitted upstreams and the models will be used solely for this challenge.
...
After training the downstream model for all tasks, we show the best performance on the hidden-set's development splits as the **practice scores** (one score per task). The true performance on the testing splits, termed **private scores**, will be revealed after announcement. The final team ranking will depend only on the hidden-set's **private scores**.

#### How to submit

(Update 04/09/2022 AOE)

The [hidden-set leaderboard](leaderboard?subset=Hidden+Dev+Set&track=constrained) currently is under maintenance and not accepts submissions. 

<!-- Please follow [the submission steps](https://huggingface.co/superb/superb-submission). We use HuggingFace's Hub to host the submitted upstreams and track the submitted model weights. On the other hand, we use our [submission page](submit?type=hidden) to control the submission limit, where the participants tell us the locations of their models on HuggingFace's Hub.
...
We announce two kinds of metrics to measure model performance: **superb-rank** ($superb_r$) and **superb-score** ($superb_s$) in parameter-agnostic manner. In this challenge, $superb_r$ is the primary metrics. When equal rank is found on two different upstreams, $superb_{s}$ is used to break the tie.

To encourage the development of speech SSL on small and green models, we add two metric, **Params** for memory usage and **MACs** for computataional cost as another targets to make improvement. Also, to gauge the effectiveness and parameter-efficiency of new SSL algorithms, we encourage participants to submit multiple upstreams trained by the same algorithm with the only difference in parameter size. They are computed by [this profiling tool](https://github.com/B06901052/DeepSpeed/tree/master/deepspeed/profiling/flops_profiler) modified from deepspeed.
...
The public-set is for the upstream development purpose. You can pre-train your upstream and evaluate it with any method you like. You are required to submit at least one upstream model to the hidden-set leaderboard. The hidden-set leaderboard submission deadline is **Jul 15, 2022** (tmp).

### Submission selection

A team can **select at most submissions** among its previous submissions for the final team ranking. However, these 2 submissions **must come from the same method** and only differ in parameter size. The deadline for the submission selection is **Jul 18, 2022** (tmp). If you want to submit more models come from different method, just create more team. 

### System description paper (Optional)

To verify the submitted upstream follows the challenge policy, we suggest to submit a system description paper in **SLT submission format** without the page limit. The paper should describe the method **for the selected submissions**, containing at least the following materials:
...
## Inviter Announcement and Presentation (not sure whether say that or not)

After review the system description papers and compare their performance with the hidden-set **private scores**. We will reveal all the private scores and announce the final results on **January 01, 2023** (tmp). And the outstanding one may be invited to present their methods in SLT workshop.

## Timeline

- Apr 15, 2022: Challenge announcement
- Apr 15, 2022: Public-set submission start
- Apr 30, 2022: Hidden-set submission start
- Jul, 2022: Submission deadline
- Jan, 2023 : Results announcement and hosting challenge
...
export { policy_aaai2022, policy_slt2022 }
