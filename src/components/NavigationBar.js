...
function LiftingBarButton(props) {
  const classes = useStyles();
  return (
    <div className={`${classes.tool} ${classes.button}`}>{props.children}</div>
  );
}
...
function NavigationBar({ width, tableControlRef, ...props }) {
  ...
  let items = [
    ...
    ["Code", "https://github.com/s3prl/s3prl/tree/master/downstream"],
    ["Tasks", "/tasks"],
    ["Submit", "/submit"],
    // ["Compare", "/compare"],
                    ...
                    {items.map(([content, link]) => (
                      <Grid item>
                        <AdaptiveLink link={link}>
                          <LiftingBarButton>{content}</LiftingBarButton>
                        </AdaptiveLink>
                      </Grid>
                    ))}
                  </Hidden>
