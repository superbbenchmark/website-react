...
dscore_dir=$inference_dir/truth/sd_public/dscore
...
for med in 1 11; do
    for th in 0.3 0.4 0.5 0.6 0.7; do
        python3 $inference_dir/make_rttm.py --median=$med --threshold=$th \
            --frame_shift=${frame_shift} --subsampling=1 --sampling_rate=${sr} \
            $scoring_log_dir/file_list $scoring_dir/hyp_${th}_$med.rttm
        python3 ${dscore_dir}/score.py -r ${test_set}/rttm -s $scoring_dir/hyp_${th}_$med.rttm \
